...
final class DeviceLockWebSocketService {
    ...
    private func connectUsingCurrentBase() {
        ...
        let urlString = "\(base)\(AppConfig.websocketTokenPath)/children/device/\(dsn)/global_application_lock/"
    ...
    private func receiveLoop(baseIndex: Int) {
        task?.receive { [weak self] result in
            ...
            case let .success(message):
                ...
                switch message {
                case let .data(data):
                    self?.handleIncoming(data: data, isReconnectDelivery: isReconnectDelivery)
                case let .string(string):
                    self?.handleIncoming(data: Data(string.utf8), isReconnectDelivery: isReconnectDelivery)
                @unknown default:
                    break
                }
    ...
    private func handleIncoming(data: Data, isReconnectDelivery: Bool) {
        guard let isLocked = parser.parse(from: data) else { return }

        DispatchQueue.main.async { [weak self] in
            self?.onGlobalLockStatusChange?(isLocked, isReconnectDelivery)
        }
    }
    ...
    private let parser = DeviceGlobalLockPayloadParser()
