...
final class DeviceApplicationLockWebSocketService {
    ...
    private func connectUsingCurrentBase() {
        ...
        let urlString = "\(base)\(AppConfig.websocketTokenPath)/children/device/\(dsn)/applications/lock"
    ...
    private func handleIncoming(data: Data, isReconnectDelivery: Bool) {
        ...
        DispatchQueue.main.async { [weak self] in
            self?.onLockEvent?(event, isReconnectDelivery)
        }
    }
    ...
    private let parser = DeviceApplicationLockPayloadParser()
