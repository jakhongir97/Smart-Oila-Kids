...
struct ChatView: View {
    ...
    private var flatMessages: [Datum] {
        viewModel.sortedKeys
            ...
            .sorted(by: { compareChatTimestamps($0.time, $1.time) == .orderedAscending })
    }
...
private func compareChatTimestamps(_ lhs: String, _ rhs: String) -> ComparisonResult {
    if let lhsDate = parseChatDate(lhs), let rhsDate = parseChatDate(rhs) {
...
private func parseChatDate(_ value: String) -> Date? {
    ...
    if let date = chatISODateFormatterWithFractional.date(from: normalized) {
        return date
    }
    if let date = chatISODateFormatter.date(from: normalized) {
        return date
    }
    return chatPlainDateFormatter.date(from: normalized)
}

private let chatISODateFormatterWithFractional: ISO8601DateFormatter = {
    let formatter = ISO8601DateFormatter()
    formatter.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
    return formatter
}()

private let chatISODateFormatter: ISO8601DateFormatter = {
    let formatter = ISO8601DateFormatter()
    formatter.formatOptions = [.withInternetDateTime]
    return formatter
}()

private let chatPlainDateFormatter: DateFormatter = {
