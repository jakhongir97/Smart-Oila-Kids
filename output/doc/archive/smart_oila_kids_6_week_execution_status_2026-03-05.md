...
| --- | --- | --- | --- | --- |
| 1 | Scope and contract freeze | DONE | Child-only extraction spec in repo (`CHILD_ONLY_EXTRACTION_SPEC.md`), OpenAPI baseline + audit scripts (`scripts/check_child_openapi_baseline.py`, `scripts/audit_parent_child_endpoints.sh`), Week 1 triage closure in `output/doc/archive/week1_members_devices_api_triage_2026-03-12.md`, child `front_camera` websocket support, Firebase token readback diagnostics | Re-open only if backend makes parent-tagged member/device contracts authoritative for child |
