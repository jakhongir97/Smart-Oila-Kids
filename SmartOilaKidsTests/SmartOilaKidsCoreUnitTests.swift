...
final class TelemetryPairingLossTests: XCTestCase {
    ...
    // MARK: A restored lock with no stamp

    func testAnUnstampedRestoredLockGetsItsCeilingEvenWhenThePollIsRefused() async throws {
        // Persisted by a build older than the confirmation stamp: locked, no stamp, no end.
        UserDefaults.standard.set(true, forKey: "OILA_LAST_LOCK_STATE")
        let restoredAt = Date()
        let refused = apiError(401, "UNAUTHORIZED")
        let stub = Stub(lockAnswers: [refused], statusError: refused)
        let (service, _, invalidations) = start(stub)
        defer { service.stop() }

        let polled = await waitUntil { stub.lockStateCalls >= 1 }
        try? await Task.sleep(nanoseconds: 200_000_000)

        XCTAssertTrue(polled)
        XCTAssertEqual(invalidations.value, 0)
        XCTAssertTrue(service.isLocked, "honoured, not unlocked on upgrade")
        let deadline = try XCTUnwrap(service.lockDeadline,
                                     "a poll that never succeeds must not leave the lock with no end")
        XCTAssertEqual(deadline.timeIntervalSince(restoredAt), OilaTelemetryService.lockRestoreMaxAge, accuracy: 5)
    }
...
final class LockRestoreCeilingTests: XCTestCase {
    ...
    func testALockFromBeforeThisFieldExistedIsHonouredOnce() {
        // An app updated while the device was locked has no stamp. Trust it; `init` then stamps it so
        // the 8 h ceiling applies (see OilaTelemetryServiceLockDeadlineTests).
        XCTAssertTrue(OilaTelemetryService.restoredLockIsTrustworthy(
...
final class OilaTelemetryServiceLockDeadlineTests: XCTestCase {
    ...
    func testAnUnstampedPersistedLockIsHonouredAndGivenTheCeilingOnce() throws {
        // A lock persisted by a build older than the confirmation stamp: no stamp, no end. It had no
        // deadline at all, so only a server answer could release it.
        UserDefaults.standard.set(true, forKey: "OILA_LAST_LOCK_STATE")
        let restoredAt = Date()

        let service = makeService()

        XCTAssertTrue(service.isLocked, "honoured rather than unlocked on upgrade")
        let deadline = try XCTUnwrap(service.lockDeadline)
        XCTAssertEqual(deadline.timeIntervalSince(restoredAt), OilaTelemetryService.lockRestoreMaxAge, accuracy: 5)
        XCTAssertGreaterThan(UserDefaults.standard.double(forKey: OilaTelemetryService.lockConfirmedAtKey), 0,
                             "persisted, so a relaunch keeps the ceiling instead of restarting it")

        let relaunched = makeService()
        let relaunchedDeadline = try XCTUnwrap(relaunched.lockDeadline)
        XCTAssertEqual(relaunchedDeadline.timeIntervalSince1970, deadline.timeIntervalSince1970, accuracy: 0.01)
    }
