...
find_built_app() {
  local app_name="$1"
  local app_path="$DERIVED_DIR/Build/Products/Debug-iphonesimulator/$app_name.app"
  if [[ -d "$app_path" ]]; then
    echo "$app_path"
    return 0
  fi
  ...
  local fallback
  fallback="$(find "$DERIVED_DIR/Build/Products/Debug-iphonesimulator" -maxdepth 1 -name '*.app' | head -n 1)"
  if [[ -n "${fallback:-}" ]]; then
    echo "$fallback"
    return 0
  fi
...
install_and_launch() {
  ...
  if [[ -z "${bundle_id:-}" ]]; then
    echo "Could not read bundle identifier: $app_path" >&2
    exit 1
  fi
...
build_app "$PARENT_PROJECT" "$PARENT_SCHEME" "$PARENT_SIM_ID" /tmp/smartoila_parent_build.log
PARENT_APP_PATH="$(find_built_app "$PARENT_APP_NAME")"
...
build_app "$CHILD_PROJECT" "$CHILD_SCHEME" "$CHILD_SIM_ID" /tmp/smartoila_child_build.log
CHILD_APP_PATH="$(find_built_app "$CHILD_APP_NAME")"
